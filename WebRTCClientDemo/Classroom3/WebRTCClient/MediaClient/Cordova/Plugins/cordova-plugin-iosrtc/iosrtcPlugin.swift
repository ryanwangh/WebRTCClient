...
class iosrtcPlugin : CDVPlugin {
    ...
    var isDownVideo1: Int  = 0  {
        didSet{
            for renderers in pluginMediaStreamRenderers.values {
                renderers.isDownVideo = isDownVideo1
            }
        }
...
//        session.overrideOutputAudioPort(AVAudioSessionPortOverride.Speaker)
	...
	func RTCPeerConnection_addStream(_ command: CDVInvokedUrlCommand) {
		...
		(self.queue).async { [weak pluginRTCPeerConnection, weak pluginMediaStream] in
            pluginRTCPeerConnection?.addStream(pluginMediaStream!)
	...
	func new_MediaStreamRenderer(_ command: CDVInvokedUrlCommand) {
        ...
        pluginMediaStreamRenderer.isDownVideo = self.isDownVideo1
