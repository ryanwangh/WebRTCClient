...
class PluginMediaStreamRenderer : NSObject, RTCEAGLVideoViewDelegate {
    ...
    var isDownVideo:Int = 0 {
        didSet{
            if isDownVideo == 0 {
                self.webView.addSubview(self.elementView)
                self.webView.bringSubview(toFront: self.elementView)
            }else{
                if !self.webView.subviews.isEmpty {
                    self.webView.subviews[0].insertSubview(self.elementView, at: 0)
                }
            }
        }
    }
...
//        self.elementView.backgroundColor = UIColor.redColor()
...
//        NSLog("__count___%@", (self.pluginMediaStream?.videoTracks.count)!)
         NSLog("__getJSON__%@", (self.pluginMediaStream?.getJSON())!)
        NSLog("___FITST__%@", (self.pluginMediaStream?.videoTracks.first.debugDescription)!)
